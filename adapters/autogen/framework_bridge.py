...
class Trial:
    ...
    async def turn(self, turn_id: str, user_msg: str) -> dict:
        ...
        # the autogen ChatCompletionClient, the openai responses bypass,
        # AND fastmcp via _httpx_factory (the factory builds clients from
        # this dict freshly each session, so it's already fine — but the
        # primary client must be mutated live).
    ...
    def _compact_responses(self, strategy: str) -> dict:
        ...
        For state=T chain mode: "drop_half" drops the older half;
        "drop_tool_calls" / "summarize" also drop half since there are no
        tool-call messages or text to summarize at this layer.
        """
        api = self.config.get("api")
