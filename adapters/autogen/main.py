"""FastAPI adapter service wrapping autogen-agentchat (AssistantAgent +
autogen-ext model clients) plus a direct openai SDK bypass for the
Responses API (which autogen-ext does not yet model).

Supports api in {chat, messages, responses, responses+conv}:
  - chat            → OpenAIChatCompletionClient (ollama/chatgpt/gemini/mock)
  - messages        → AnthropicChatCompletionClient (claude)
...
This is the first aiplay adapter to cover all 4 APIs + the state-mode
variant; that unlocks verdict (e) testing in T11.
"""
...
try:
    ...
    _AUTOGEN_VERSION = getattr(autogen_agentchat, "__version__", "unknown")
except Exception:
    _AUTOGEN_VERSION = "unknown"

from framework_bridge import Trial

log = logging.getLogger("aiplay.adapter.autogen")

app = FastAPI(title="aiplay-adapter-autogen")
...
def create_trial(req: CreateTrialReq):
    if req.config.api not in SUPPORTED_APIS:
        raise HTTPException(
            ...
            f"(autogen adapter supports {', '.join(SUPPORTED_APIS)})",
        )
    try:
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("ADAPTER_PORT", "5014"))
