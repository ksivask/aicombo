      ...
      <div class="cid-flow-legend">
        ...
        <details class="cid-flow-help">
          ...
          <div class="cid-flow-help-body">
            ...
            ${showRunLineage ? `<p><strong>Run lineage</strong> (overlay on): a dashed edge labelled <code>|run|</code> = <code>parent_rid</code> link (run → its parent run); a dashed edge labelled <code>|calls|</code> = <code>parent_run_rid</code> (the run that ORIGINATED this <code>tool_call</code>/<code>tool_response</code> — the primary MCP-call↔run association); each <code>llm_request</code> node's label gains its <code>rid</code> on a new line; a red dashed border marks a run with <code>parent_rid_anomaly</code> (carriers disagreed on the parent).</p>` : ""}
          </div>
        </details>
      </div>
      ...
      <div class="cid-flow-legend">
        <details class="cid-flow-help">
          ...
          <div class="cid-flow-help-body">
            ...
            ${showRunLineage ? `<p><strong>Run lineage</strong> (overlay on): <span style="color:#fd7e14;font-weight:bold">╌╌▶</span> dashed orange edge = <code>parent_rid</code> link (run → its parent run); <span style="color:#20c997;font-weight:bold">╌╌▶</span> dashed teal "calls" edge = <code>parent_run_rid</code> (the run that ORIGINATED this <code>tool_call</code>/<code>tool_response</code> — the primary MCP-call↔run association); each <code>llm_request</code> node's label gains its <code>rid</code>; a <span style="color:#dc3545;font-weight:bold">red dashed border</span> marks a run whose carriers disagreed on the parent (<code>parent_rid_anomaly</code>).</p>` : ""}
          </div>
        </details>
      </div>
    </div>
