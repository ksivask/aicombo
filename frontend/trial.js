...
function renderCidFlowTab(trial) {
  ...
  // E20 — Edges: audit → SS (solid for tools_list = "produced this
  // snapshot"; thick for tools_call = "carried this snapshot").
  // Visualizes snapshot correlation — operators can read at a glance
  // whether each tools_list snapshot got picked up by a downstream
  // tools_call (consumed) or sat unused (orphan).
  for (const x of ssAudits) {
    if (x.phase === "tools_list") {
      mer += `  A${x.i} --> ${ssNodeId(x.ss)}\n`;
      ...
      <div class="cid-flow-legend">
        ...
        <details class="cid-flow-help">
          ...
          <div class="cid-flow-help-body">
            ...
            <p>Asymmetric flag-shaped nodes representing tools/list snapshot identities (8-hex SHA-256 prefix). Edges from <code>tools_list</code> audits show "this list-call produced this snapshot"; thick edges from <code>tool_call</code> audits show "this call carried this snapshot back" (E20 correlation succeeded).</p>
            <ul>
