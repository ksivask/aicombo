...
function buildColumnDefs() {
  return [
    ...
    },
    {
      ...
      // CRITICAL: cellDataType: false disables AG-Grid v32's data-type
      // inference. Without it, AG-Grid infers "text" from the initial
      // string value and REJECTS array commits (silently reverting to
      // the old single value). Disabling lets the column hold either a
      // string OR list[string] depending on user selection.
      cellDataType: false,
    ...
    },
    {
      ...
      // list-form against MULTI_MCP_FRAMEWORKS server-side (E24a opted
      // combo in; previously the set was empty pending per-adapter
      // implementation).
      // cellDataType: false — same reason as the LLM column above.
      // Without this, AG-Grid v32 silently rejects array commits.
      cellDataType: false,
      cellEditor: MultiSelectCellEditor,
      cellEditorPopup: true,
